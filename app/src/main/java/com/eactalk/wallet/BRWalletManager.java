...
import com.eactalk.presenter.entities.BRMerkleBlockEntity;
import com.eactalk.presenter.entities.BRPeerEntity;
import com.eactalk.presenter.entities.BRTransactionEntity;
import com.eactalk.presenter.entities.Fee;
import com.eactalk.presenter.entities.ImportPrivKeyEntity;
import com.eactalk.presenter.entities.TxItem;
import com.eactalk.presenter.interfaces.BROnSignalCompletion;
import com.eactalk.tools.animation.BRAnimator;
import com.eactalk.tools.animation.BRDialog;
import com.eactalk.tools.animation.SpringAnimator;
import com.eactalk.tools.manager.AnalyticsManager;
import com.eactalk.tools.manager.BRNotificationManager;
import com.eactalk.tools.manager.BRSharedPrefs;
import com.eactalk.tools.manager.FeeManager;
import com.eactalk.tools.security.BRKeyStore;
import com.eactalk.tools.sqlite.MerkleBlockDataSource;
import com.eactalk.tools.sqlite.PeerDataSource;
import com.eactalk.tools.sqlite.TransactionDataSource;
import com.eactalk.tools.threads.BRExecutor;
import com.eactalk.tools.threads.ImportPrivKeyTask;
import com.eactalk.tools.util.BRConstants;
import com.eactalk.tools.util.BRCurrency;
import com.eactalk.tools.util.BRExchange;
import com.eactalk.tools.util.Bip39Reader;
import com.eactalk.tools.util.TypesConverter;
...
//                    builder.setTitle("password protected key");
    ...
    public static void onTxAdded(byte[] tx, int blockHeight, long timestamp, final long amount, String hash) {
        ...
        final Context ctx = EactalkApp.getBreadContext();
        if (amount > 0) {
            BRExecutor.getInstance().forMainThreadTasks().execute(new Runnable() {
                @Override
                public void run() {
                    String am = BRCurrency.getFormattedCurrencyString(ctx, "EAC", BRExchange.getBitcoinForSatoshis(ctx, new BigDecimal(amount)));
                ...
                public void run() {
                    if (!BRToast.isToastShown()) {
                        BRToast.showCustomToast(finalCtx, message,
                                EactalkApp.DISPLAY_HEIGHT_PX / 2, Toast.LENGTH_LONG, R.drawable.toast_layout_black);
    ...
    public static void onTxUpdated(String hash, int blockHeight, int timeStamp) {
        ...
        Context ctx = EactalkApp.getBreadContext();
        if (ctx != null) {
