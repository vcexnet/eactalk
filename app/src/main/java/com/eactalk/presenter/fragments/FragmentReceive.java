...
//        });
    ...
    private void updateQr() {
        final Context ctx = getContext() == null ? EactalkApp.getBreadContext() : (Activity) getContext();
        BRExecutor.getInstance().forLightWeightBackgroundTasks().execute(new Runnable() {
            @Override
            public void run() {
                ...
                BRExecutor.getInstance().forMainThreadTasks().execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        boolean generated = QRUtils.generateQR(ctx, "earthcoin:" + receiveAddress, mQrImage);
                        if (!generated)
